...
;	
	...
	bg_bot = where(*str_indx.sat_ind lt (*str_indx.bg_ind)[0], ct_bot)
	bg_top = where(*str_indx.sat_ind gt (*str_indx.bg_ind)[ct_bg-1], ct_top)
	...
	if ct_bot gt 0 then for i=0, ct_bot-1 do bg[*, *, bg_bot[i]] = bg_[*,*,0]
	if ct_top gt 0 then for i=0, ct_top-1 do bg[*, *, bg_top[i]] = bg_[*,*,ct_bg-1]
