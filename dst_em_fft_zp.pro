pro dst_em_fft_zp , str , cpsf , it=it , level=level 
...
tmp = fltarr(dim) & tmp[0,0] = cpsf
cpsf = shift(tmp, (dim[0]-(size(/dim,*str.im))[0])/2,(dim[1]-(size(/dim,*str.im))[1])/2);convolve(mask , cpsf)>0
cpsf2 = cpsf * cpsf
