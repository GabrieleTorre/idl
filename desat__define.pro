...
pro desat::indices_analysis, str, aec = aec
	...
	q_t_ind = anytim(info.date_obs) gt anytim(str.ts) and anytim(info.date_obs) lt anytim(str.te)
	t_ind = where( q_t_ind , ct )

	;;; saturated frame indices
	q_sat_t = info.DATAMAX ge str.sat_lev
	sat_t = where(q_sat_t)

	;;; short exposure time file indices
	q_s_exp = info.exptime lt 0.8*median(info.exptime)
	s_exp = where(q_s_exp)

	;;; indices for saturated frames in the selected time interval
	sat_ind = ~keyword_set(aec) ? where( q_t_ind and q_sat_t and ~q_s_exp , ct_sat ) : $
								  where( q_t_ind and q_sat_t , ct_sat )
					
	;;; background indices estimation over the whole dataset
	q_offset = intarr(n_el) + 1
	q_bg_ind_dataset = isarray(s_exp) ? (intarr(n_el)+1) - q_sat_t + q_s_exp ge 1 : (intarr(n_el)+1) and ~q_sat_t
	bg_ind_dataset = where(q_bg_ind_dataset , ct_bg) 
	
	;;; background indices estimation in the select time interval
	bg_ind_t_ind = where( q_t_ind and q_bg_ind_dataset , ct_bg )
 	...
 	bg_ind = bg_ind[UNIQ(bg_ind, SORT(bg_ind))]							;;;GT modification
  ...
  print, 'Background Indices: ' , bg_ind
  print, 'Saturated Frame Index: '  , sat_ind
...
function desat::deconv, index, fill = fill, it = it, lev = lev, dwavelength = dwavelength
	...
	data_obs = ((self-> get(/index))[index]).date_obs
	...
	for i=0,size(/n_e, index)-1 do begin
		...
		print, strtrim(i+1), '	of ', strtrim(size(/n_e, index)) , ': ' , data_obs[i] 
...
; data selection mask for correlation
  ...
  q_corr_fring = convolve( mask, psf_str.cpsf ) ge 1.e-3 * max(cpsf)

  corr_g = where( q_corr_fring and q_2 )
  ...
  qz = mask_z gt min(psf_str.opsf[where(psf_str.opsf gt 0)])
  z = where(qz)

  ;;; DATA (Fringe) PIXELS 
  g = where( Q_2 and qfwd_ind and ~qz )  ;;; Gt modification
	...
	for i=0, n_rec-1 do begin
      ...
      if ~psplot then window, numwin, xsize = 900, ysize = 600, title= title + text else $
		...
		*str.bg *= mult_fact
		...
		mask = *str.im * 0. & mask[*str.s] = 1
  		;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; Correlation analysis ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
		nwav = 15
		wmin = 131 - 2 & wmax = 131 + 2 & delta_w = (wmax - wmin)/float(nwav)
		obswav = 131 + ( findgen(nwav) - (nwav/2.) )
		dwavelength	= nwav gt 0 ? (obswav - 131.)/131. : 0
	
		C = FLTARR(size(/n_e, *str.s), size(/n_e, dwavelength))
	
		for iw=0, size(/n_e, dwavelength) - 1 do begin 
			psf_str = self -> psf( info[i] , dwavelength[iw] , core_dim )
		
			q_corr_fring = convolve( mask, psf_str.cpsf ) ge 1.e-3 * max(psf_str.cpsf)
			corr_g = where( q_corr_fring and q_2 )
